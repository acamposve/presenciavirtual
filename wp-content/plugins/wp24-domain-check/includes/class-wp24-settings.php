...
class WP24_Domain_Check_Settings {
	...
	public function init_settings() {
		...
		add_settings_field(
			...
			array(
				...
				'vals' => array(
					...
					'grouped'	=> __( 'Drop-Down Grouped' , 'wp24-domaincheck' ) .
						' <span class="description">(' . __( 'Select the TLD from grouped list, only TLDs listed below allowed' , 'wp24-domaincheck' ) . ')</span>',
		...
		add_settings_field(
			...
			array(
				'name' => 'tlds',
				'type' => 'textarea',
				'desc' => __( 'Comma separated list of testable TLDs without leading point.', 'wp24-domaincheck' ) . '<span id="grouped-example"><br>' .
					__( 'Grouped example: <code>[Group 1], com, net, org, [Group 2], info, eu</code>.', 'wp24-domaincheck' ) . '</span>',
			)
		);
		// multicheck
		add_settings_field(
			'multicheck',
			__( 'Multicheck', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_general',
			'section_general_tld',
			array(
				'name'	=> 'multicheck',
				'type'	=> 'checkbox',
				'label'	=> __( 'Check multiple domains separated by comma.', 'wp24-domaincheck' ),
			)
		);
		// check all
		add_settings_section( 'section_general_check_all', __( 'Check All', 'wp24-domaincheck' ), '', 'settings_general' );
		// checkAll
		add_settings_field(
			'checkAll',
		...
		add_settings_field(
			'checkAllDefault',
			__( 'Check all default', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_general',
			'section_general_check_all',
			array(
				...
				'type'	=> 'checkbox',
				'label'	=> __( 'Preselect all in drop-down list', 'wp24-domaincheck' ),
			)
		);
		// button
		...
		add_settings_section( 'section_advanced_unsupported_tld', __( 'Unsupported TLD Settings', 'wp24-domaincheck' ), '', 'settings_advanced' );
		// enabled
		add_settings_field(
			'unsupported_enabled',
			__( 'Unsupported TLDs', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_advanced',
			...
			array(
				...
				'subname'	=> 'enabled',
				'type'		=> 'checkbox',
				'label'		=> __( 'Allow checking of unsupported (no whois server available) TLDs.', 'wp24-domaincheck' ),
				'desc'		=> __( 'Check result may not be a 100% correct.',  'wp24-domaincheck' ) .
					' <a href="https://wp24.org/plugins/domain-check/faq.html">' . __( 'Read more', 'wp24-domaincheck') . '</a>.',
			)
		);
		// text / color
		add_settings_field(
		...
		if ( count( $this->webwhois_tlds ) > 0 ) {
			...
			add_settings_field(
				'unsupported_verify',
				__( 'Verification link', 'wp24-domaincheck' ),
				array( $this, 'inputfield' ),
				'settings_advanced',
				'section_advanced_unsupported_tld',
				array(
					...
					'subname'	=> 'verify',
					'type'		=> 'checkbox',
					'label'		=> __( 'Show external whois page link to verify available domain.', 'wp24-domaincheck' ),
					'desc'		=> __( 'Available for', 'wp24-domaincheck' ) . ' ' . implode( ', ', $this->webwhois_tlds ),
				)
			);
			// verifyText
			add_settings_field(
				'unsupported_verifyText',
				__( 'Verification link text', 'wp24-domaincheck' ),
				array( $this, 'inputfield' ),
				'settings_advanced',
				'section_advanced_unsupported_tld',
				array(
		...
		// misc settings
		add_settings_section( 'section_advanced_misc', __( 'Misc Settings', 'wp24-domaincheck' ), '', 'settings_advanced' );
		// dotInSelect
		add_settings_field(
			'dotInSelect',
			__( 'Dot in drop-down', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_advanced',
			...
			array(
				'name'	=> 'dotInSelect',
				'type'	=> 'checkbox',
				'label'	=> __( 'Display the dot inside the drop-down list, instead of before it.', 'wp24-domaincheck' ),
			)
		);
		// useNonces
		add_settings_field(
			'useNonces',
			__( 'Use nonces', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_advanced',
			'section_advanced_misc',
			array(
				'name'	=> 'useNonces',
				'type'	=> 'checkbox',
				'desc'	=> __( 'A nonce (number used once) additionally helps to protect from misuse.<br>(Disable when using caching or compression.)', 'wp24-domaincheck' ),
			)
		);
		// multipleUse
		add_settings_field(
		...
		add_settings_section( 'section_advanced_suggestions', __( 'Alternative Suggestions', 'wp24-domaincheck' ), '', 'settings_advanced' );
		// prefixes
		add_settings_field(
			'prefixes',
			__( 'Prefixes', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_advanced',
			...
			array(
				'name' => 'prefixes',
				'type' => 'textfield',
				'desc' => __( 'Comma separated list of prefixes for alternative domain names.', 'wp24-domaincheck' ),
			)
		);
		// suffixes
		add_settings_field(
			'suffixes',
			__( 'Suffixes', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_advanced',
			'section_advanced_suggestions',
			array(
				'name' => 'suffixes',
				'type' => 'textfield',
				'desc' => __( 'Comma separated list of suffixes for alternative domain names.', 'wp24-domaincheck' ),
			)
		);

		// form settings
		add_settings_section( 'section_advanced_form', __( 'Form Settings', 'wp24-domaincheck' ), '', 'settings_advanced' );
		// htmlForm
		add_settings_field(
			'htmlForm',
			__( 'HTML form', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_advanced',
			...
			array(
				'name'	=> 'htmlForm',
				'type'	=> 'checkbox',
				'label'	=> __( 'Use HTML form with submit button.', 'wp24-domaincheck' ),
				'desc'	=> __( 'Deactivate to use a regular button without a form.', 'wp24-domaincheck' ) . '<br>' .
					__( 'Use shortcode <code>[wp24_domaincheck html_form="0"]</code> to deactivate for one specific instance.', 'wp24-domaincheck' ),
			)
		);
		// fieldnameDomain
		add_settings_field(
		...
		add_settings_field(
			'fieldnameTld',
			__( 'TLD fieldname', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_advanced',
			'section_advanced_form',
			array(
		...
		if ( count( $this->limited_tlds ) > 0 ) {
			// query limits
			add_settings_section( 'section_advanced_query_limits', __( 'Query limits', 'wp24-domaincheck' ), '', 'settings_advanced' );
		...
		// developer settings
		add_settings_section( 'section_advanced_developer', __( 'Developer Settings', 'wp24-domaincheck' ), '', 'settings_advanced' );
		// hooksEnabled
		add_settings_field(
			'hooksEnabled',
		...
		add_settings_field(
			...
			__( 'Invalid input', 'wp24-domaincheck' ),
			array( $this, 'compositefield' ),
			'settings_texts_colors',
			'section_texts_colors',
			array(
				...
				array(
					...
					'type' => 'colorfield',
					'desc' => __( 'Leave empty to use browser specific "Please match the requested format" message.', 'wp24-domaincheck' ),
				),
			)
		);
		...
		// purchase
		add_settings_section( 'section_prices_links_purchase', __( 'Purchase (for available domains)', 'wp24-domaincheck' ), '', 'settings_prices_links' );
		// priceEnabled
		add_settings_field(
			'priceEnabled',
			__( 'Purchase prices', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
		...
		add_settings_field(
			'priceDefault',
			__( 'Default purchase price', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_prices_links',
			'section_prices_links_purchase',
			array(
		...
		add_settings_field(
			'linkEnabled',
			__( 'Purchase links', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_prices_links',
			'section_prices_links_purchase',
			array(
		...
		add_settings_field(
			...
			array(
				'name' => 'textPurchase',
				'type' => 'textfield',
				'desc' => __( 'Use <strong>[price]</strong> as placeholder for the price and <strong>[link]</strong>linktext<strong>[/link]</strong> as placeholder for the link or <strong>[button]</strong>buttontext<strong>[/button]</strong> to use a button instead of a link.', 'wp24-domaincheck' ),
			)
		);

		// transfer
		add_settings_section( 'section_prices_links_transfer', __( 'Transfer (for registered domains)', 'wp24-domaincheck' ), '', 'settings_prices_links' );
		// priceTransferEnabled
		add_settings_field(
			'priceTransferEnabled',
			__( 'Transfer prices', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_prices_links',
		...
		add_settings_field(
			'priceDefault',
			__( 'Default transfer price', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_prices_links',
			'section_prices_links_transfer',
			array(
		...
		add_settings_field(
			'linkTransferEnabled',
			__( 'Transfer links', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_prices_links',
			'section_prices_links_transfer',
			array(
		...
		add_settings_field(
			'linkTransferDefault',
			__( 'Default transfer link', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_prices_links',
			'section_prices_links_transfer',
			array(
		...
		add_settings_field(
			'textTransfer',
			__( 'Transfer text', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_prices_links',
			'section_prices_links_transfer',
			array(
		...
		add_settings_field(
			'woocommerce_enabled',
			__( 'Integration', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			...
			array(
				...
				'subname'	=> 'enabled',
				'type'		=> 'checkbox',
				'label'		=> __( 'Enabled', 'wp24-domaincheck' ),
				'desc'		=> __( 'If enabled prices &amp; links will be disabled.', 'wp24-domaincheck' ),
			)
		);
		// addToCartBehaviour
		add_settings_field(
			'woocommerce_addToCartBehaviour',
			__( 'Add to cart behaviour', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_woocommerce',
			'section_woocommerce',
			array(
				...
				'type'		=> 'combobox',
				'vals'		=> array(
					0 => __( 'Redirect to cart page', 'wp24-domaincheck' ),
					1 => __( 'Stay on current page (links)', 'wp24-domaincheck' ),
					3 => __( 'Stay on current page (checkboxes)', 'wp24-domaincheck' ),
					2 => __( 'Redirect to custom page', 'wp24-domaincheck' ),
				),
			)
		);
		// customPageLink
		add_settings_field(
			...
			__( 'Custom page link', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_woocommerce',
			'section_woocommerce',
			array(
		...
		add_settings_field(
			...
			__( 'Add to cart text', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_woocommerce',
			'section_woocommerce',
			array(
		...
		add_settings_field(
			...
			__( 'Added to cart text', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_woocommerce',
			'section_woocommerce',
			array(
				...
				'type'		=> 'textfield',
				'desc'		=> __( 'Change link or button text to this text after adding domain to cart.', 'wp24-domaincheck' ),
			)
		);
		// domainLabel
		add_settings_field(
			'woocommerce_domainLabel',
			__( 'Domain label', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_woocommerce',
			'section_woocommerce',
			array(
				...
				'subname'	=> 'domainLabel',
				'type'		=> 'textfield',
				'desc'		=> __( 'Labeling in front of the domain, under the product name in the shopping cart and the order.', 'wp24-domaincheck' ),
			)
		);
		
		// purchase
		add_settings_section( 'section_woocommerce_purchase', __( 'Purchase (for available domains)', 'wp24-domaincheck' ), '', 'settings_woocommerce' );
		// productidPurchase
		add_settings_field(
			'woocommerce_productidPurchase',
			__( 'Purchase product', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_woocommerce',
		...
		// transfer
		add_settings_section( 'section_woocommerce_transfer', __( 'Transfer (for registered domains)', 'wp24-domaincheck' ), '', 'settings_woocommerce' );
		// transferEnabled
		add_settings_field(
			'woocommerce_transferEnabled',
			__( 'Transfer', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_woocommerce',
		...
		add_settings_field(
			...
			__( 'Transfer product', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_woocommerce',
			'section_woocommerce_transfer',
			array(
		...
		add_settings_field(
			...
			__( 'Transfer suffix', 'wp24-domaincheck' ),
			array( $this, 'inputfield' ),
			'settings_woocommerce',
			'section_woocommerce_transfer',
			array(
				...
				'type'		=> 'textfield',
				'desc'		=> __( 'Suffix after the domain, under the product name in the shopping cart and the order.', 'wp24-domaincheck' ),
			)
		);
	...
	public function get_html() {
		...
		$tabs = array();
		$tabs['general'] = __( 'General Settings', 'wp24-domaincheck' );
		$tabs['advanced'] = __( 'Advanced Settings', 'wp24-domaincheck' );
		$tabs['texts_colors'] = __( 'Texts &amp; Colors', 'wp24-domaincheck' );
		$tabs['prices_links'] = __( 'Prices &amp; Links', 'wp24-domaincheck' );
		$tabs['woocommerce'] = __( 'WooCommerce', 'wp24-domaincheck' );
		...
		if ( 'none' != $this->options['recaptcha']['type'] && ( empty( $this->options['recaptcha']['siteKey'] ) || empty( $this->options['recaptcha']['secretKey'] ) ) ) {
			...
			echo __( 'reCAPTCHA is enabled, but no site or secret key is set.', 'wp24-domaincheck' );
			echo '</p></div>';
		}
		...
		if ( $this->options['reviewMessage'] ) {
			if ( isset( $_GET['dismiss-review-message'] ) ) {
				...
				printf(
					__( 'If you like WP24 Domain Check, <a href="%1$s">click here</a> to leave a positive review. If you do not like it or have suggestions for improvement <a href="%2$s">click here</a> to tell us.', 'wp24-domaincheck' ),
				...
				echo ' <a href="' . admin_url( 'options-general.php?page=wp24_domaincheck_settings&tab=' . $active_tab ) . '&dismiss-review-message">' . __( 'Dismiss', 'wp24-domaincheck' ) . '</a>.';
				echo '</p></div>';
			}
		}
	...
	public function validate( $input ) {
		...
		foreach ( $input as $key => $val ) {
			// skip boolean fields
			if ( in_array( $key, array( 'checkAll', 'checkAllDefault', 'multicheck', 'showWhois', 'dotInSelect', 'useNonces', 'multipleUse', 'removeWhoisComments', 'htmlForm', 'hooksEnabled', 'priceEnabled', 'linkEnabled', 'priceTransferEnabled', 'linkTransferEnabled' ) ) )
				continue;
				...
				if ( empty( trim( $val ) ) )
					...
					if ( ! $this->options['unsupported']['enabled'] ) {
						add_settings_error( 'tlds', esc_attr( 'settings_error' ), implode( ', ', $unsupported_tlds ) .
							' ' . __( 'not supported', 'wp24-domaincheck' ) . '.<br>' .
							__( 'Hint: Enable "Unsupported TLDs" in the advanced settings.', 'wp24-domaincheck' ) );
					}
					else {
						add_settings_error( 'tlds', esc_attr( 'settings_error' ), __( 'No whois server available for', 'wp24-domaincheck' ) .
							' ' . implode( ', ', $unsupported_tlds ) . '.<br>' .
							__( 'Check result may not be a 100% correct.', 'wp24-domaincheck' ) .
							' <a href="https://wp24.org/plugins/domain-check/faq.html">' . __( 'Read more', 'wp24-domaincheck') . '</a>.', 'warning' );
					}
				}
	...
	private function prices_links() {
		...
		if ( $this->options['woocommerce']['enabled'] ) {
			echo '<p>' . __( 'WooCommerce integration enabled, disable to use prices &amp; links.', 'wp24-domaincheck' ) . '</p>';
			return;
		}
		...
		echo '<h2 class="title">' . __( 'Individual TLD settings', 'wp24-domaincheck' ) . '</h2>';
		...
		if ( $this->options['linkEnabled'] || $this->options['linkTransferEnabled'] )
			echo '<th>' . __( 'Link', 'wp24-domaincheck' ) . '</th>';
		echo '<th>&nbsp;</th>';
		echo '</tr></thead>';
		...
		if ( $this->options['priceEnabled'] || $this->options['priceTransferEnabled'] ) {
			...
			if ( $this->options['priceEnabled'] )
				echo '<div>' . __( 'Purchase price', 'wp24-domaincheck' ) . ':<br><input type="text" name="price" maxlength="25"></div>';
			if ( $this->options['priceTransferEnabled'] )
				echo '<div>' . __( 'Transfer price', 'wp24-domaincheck' ) . ':<br><input type="text" name="price_transfer" maxlength="25"></div>';
		...
		if ( $this->options['linkEnabled'] || $this->options['linkTransferEnabled'] ) {
			...
			if ( $this->options['linkEnabled'] )
				echo '<div>' . __( 'Purchase link', 'wp24-domaincheck' ) . ':<br><input type="text" class="regular-text" name="link"></div>';
			if ( $this->options['linkTransferEnabled'] )
				echo '<div>' . __( 'Transfer link', 'wp24-domaincheck' ) . ':<br><input type="text" class="regular-text" name="link_transfer"></div>';
			echo '<p class="description">' . __( 'Leave empty to use default link.', 'wp24-domaincheck' ) . '<br>' . __( 'Use <strong>[domain]</strong> as placeholder for the domain name.', 'wp24-domaincheck' ) . '</p>';
			echo '</td>';
		}
		echo '<td>';
		echo '<input type="submit" name="save" class="button button-primary" value="' . __( 'Save', 'wp24-domaincheck' ) . '">';
		echo '&nbsp;';
		echo '<input type="submit" name="delete" class="button button-secondary" value="' . __( 'Delete', 'wp24-domaincheck' ) . '">';
		echo '</td>';
		echo '</tr>';
		...
		foreach ( $rows as $row ) {
			...
			if ( $this->options['priceEnabled'] || $this->options['priceTransferEnabled'] ) {
				echo '<td>';
				if ( $this->options['priceEnabled'] )
					echo '<div>' . __( 'P', 'wp24-domaincheck' ) . ': ' . ( empty( $price ) ? '<span class="dashicons dashicons-warning"></span>' : $price ) . '</div>';
				if ( $this->options['priceTransferEnabled'] )
					echo '<div>' . __( 'T', 'wp24-domaincheck' ) . ': ' . ( empty( $price_transfer ) ? '<span class="dashicons dashicons-warning"></span>' : $price_transfer ) . '</div>';
				echo '</td>';
			}
			if ( $this->options['linkEnabled'] || $this->options['linkTransferEnabled'] ) {
				echo '<td>';
				if ( $this->options['linkEnabled'] )
					echo '<div>' . __( 'P', 'wp24-domaincheck' ) . ': ' . ( empty( $link ) ? '<span class="dashicons dashicons-warning"></span>' : $link ) . '</div>';
				if ( $this->options['linkTransferEnabled'] )
					echo '<div>' . __( 'T', 'wp24-domaincheck' ) . ': ' . ( empty( $link_transfer ) ? '<span class="dashicons dashicons-warning"></span>' : $link_transfer ) . '</div>';
				echo '</td>';
			}
			echo '<td><a href="javascript: void(0);" onclick="editTldPriceLink(\'' . $tld . '\', \'' . $price . '\', \'' . $link . '\', \'' . $price_transfer . '\', \'' . $link_transfer . '\'); return false;">' . __( 'Edit', 'wp24-domaincheck' ) . '</a></td>';
			echo '</tr>';
		}
		echo '</table>';
		echo '</form>';
	...
	private function woocommerce() {
		...
		if ( ! class_exists( 'woocommerce' ) ) {
			echo '<p>' . __( 'WooCommerce plugin is not activated or installed.', 'wp24-domaincheck' ) . '</p>';
			return;
		}
		...
		echo '<thead><tr>';
		echo '<th>' . __( 'TLD', 'wp24-domaincheck' ) . '</th>';
		echo '<th>' . __( 'Purchase product', 'wp24-domaincheck' ) . '</th>';
		if ( $this->options['woocommerce']['transferEnabled'] )
			echo '<th>' . __( 'Transfer product', 'wp24-domaincheck' ) . '</th>';
		echo '<th>&nbsp;</th>';
		echo '</tr></thead>';
		...
		foreach ( $rows as $row ) {
			...
			echo '<td><a href="javascript: void(0);" onclick="editTldWooCommerce(\'' . $tld . '\', ' . $product_id_purchase . ', ' . $product_id_transfer . '); return false;">' . __( 'Edit', 'wp24-domaincheck' ) . '</a></td>';
			echo '</tr>';
		}
		echo '</table>';
		echo '</form>';
	...
	private function whoisservers() {
		...
		echo '<h2 class="title">' . __( 'Whois Servers', 'wp24-domaincheck' ) . '</h2>';
			...
			if ( isset( $_POST['save'] ) ) {
				...
				if ( ! empty( $host ) && ! empty( $status_free ) ) {
					if ( isset( $whoisservers[ $tld ] ) ) {
						echo '<div class="notice notice-error is-dismissible"><p>';
						echo __( 'TLD already exists in the normal whois server list.', 'wp24-domaincheck' );
						echo '</p></div>';
					}
					else {
		...
		echo '<h2 class="title">' . __( 'Custom Whois Servers', 'wp24-domaincheck' ) . '</h2>';
		echo '<form action="' . $_SERVER['REQUEST_URI'] . '" method="post">';
		echo '<table class="wp-list-table widefat striped">';
		echo '<thead><tr>';
		echo '<th>' . __( 'TLD', 'wp24-domaincheck' ) . '</th>';
		echo '<th>' . __( 'Whois server', 'wp24-domaincheck' ) . '</th>';
		echo '<th>' . __( 'Status free', 'wp24-domaincheck' ) . '</th>';
		echo '<th>&nbsp;</th>';
		echo '</tr></thead>';
		...
		foreach ( $rows as $row ) {
			...
			echo '<td><a href="javascript: void(0);" onclick="editTldWhoisServers(\'' . $tld . '\', \'' . $host . '\', \'' . $status_free . '\'); return false;">' . __( 'Edit', 'wp24-domaincheck' ) . '</a></td>';
			echo '</tr>';
		}
		echo '</table>';
		echo '</form>';
	...
	private function about() {
		...
		echo '<tr><th>' . __( 'F.A.Q.', 'wp24-domaincheck' ) . '</th><td><a href="https://wp24.org/plugins/domain-check/faq.html">https://wp24.org/plugins/domain-check/faq.html</a></td></tr>';
		echo '<tr><th>' . __( 'Donate', 'wp24-domaincheck' ) . '</th><td><a href="https://wp24.org/donate">https://wp24.org/donate</a></td></tr>';
		echo '</table>';
